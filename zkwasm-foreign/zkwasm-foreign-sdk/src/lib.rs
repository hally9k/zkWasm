...
use zkwasm_types::host_function::{HostFunctionDesc, HostPlugin};
...
struct Function {
    ...
    signature: zkwasm_types::host_function::Signature,
}
...
pub(self) trait BuiltInHostFunction {
    ...
    fn signature() -> zkwasm_types::host_function::Signature;
...
impl HostEnv {
    ...
    pub fn register_function(
        ...
        handler: Box<dyn Fn(&mut dyn ForeignContext, RuntimeArgs) -> Option<RuntimeValue>>,
        plugin: HostPlugin,
    ) -> Result<usize, zkwasm_types::host_function::Error> {
        if self.functions.get(name).is_some() {
            return Err(zkwasm_types::host_function::Error::DuplicateRegister);
        }
    ...
    fn check_signature(&self, index: usize, signature: &Signature) -> bool {
        ...
        let signature: zkwasm_types::host_function::Signature = signature.clone().into();
